...
beforeAll(async () => {
  ...
  await db.query(`
    ...
    CREATE TABLE app.imports (id int GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY);
...
describe("generateTypes, supabase shape", () => {
  ...
  it("makes columns Postgres will not take a value for unwritable", async () => {
    // A generated column and an identity column GENERATED ALWAYS refuse an explicit value
    // ("cannot insert a non-DEFAULT value"). Typed `?: T`, `insert({ search: "x" })` typechecked
    // and failed at runtime; `?: never` says no at compile time instead.
    const { text } = await generateTypes({ databaseUrl: db.url });
    const block = (table: string, part: "Insert" | "Update") =>
      new RegExp(
        `            ${table}: \\{[\\s\\S]*?                ${part}: \\{\\n([\\s\\S]*?)\\n                \\};`,
      ).exec(text)?.[1] ?? "";
    expect(block("orders", "Insert")).toContain("                    search?: never;");
    expect(block("orders", "Update")).toContain("                    search?: never;");
    expect(block("accounts", "Insert")).toContain("                    id?: never;");
    expect(block("accounts", "Update")).toContain("                    id?: never;");
    // BY DEFAULT identity takes an explicit value, so it stays writable.
    expect(block("imports", "Insert")).toContain("                    id?: number;");
    // What SELECT returns does not change.
    expect(text).toContain("                    search: string | null;");
  });
